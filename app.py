...
# ==================== KEMENDIKDASMEN THEME (CSS) ====================
# Palet warna mengikuti gaya Kemendikdasmen (biru dominan + aksen kuning/oranye)
KMD_BLUE = "#067ac1"
KMD_ACCENT = "#f9a703"
KMD_BG = "#f5f7fb"
KMD_TEXT = "#0f172a"
...
:root {{
  --kmd-blue: {KMD_BLUE};
  --kmd-accent: {KMD_ACCENT};
  --kmd-bg: {KMD_BG};
  --kmd-text: {KMD_TEXT};
  --kmd-muted: {KMD_MUTED};
  --kmd-border: {KMD_BORDER};
  --kmd-card: {KMD_CARD};
}}

html, body {{
  background: var(--kmd-bg) !important;
}}
...
small, .kmd-muted {{
  color: var(--kmd-muted) !important;
}}

/* --- Topbar (header) --- */
.kmd-topbar {{
  background: linear-gradient(90deg, var(--kmd-blue), #0b5fa8);
  border: 1px solid rgba(255,255,255,.18);
  border-radius: 18px;
  padding: 16px 18px;
  color: white;
  box-shadow: 0 16px 30px rgba(6,122,193,.18);
  margin-bottom: 14px;
}}
.kmd-topbar .title {{
  font-size: 18px;
...
[data-testid="stSidebar"] {{
  ...
  border-right: 1px solid var(--kmd-border);
}}

.kmd-brand {{
  background: linear-gradient(135deg, rgba(6,122,193,.10), rgba(249,167,3,.10));
  border: 1px solid rgba(6,122,193,.18);
  border-radius: 18px;
  padding: 16px 14px;
  margin-top: 6px;
}}
...
div[role="radiogroup"] > label:hover {{
  border-color: rgba(6,122,193,.35);
  box-shadow: 0 12px 18px rgba(6,122,193,.08);
  transform: translateY(-1px);
}}

/* selected radio (best-effort; Streamlit internal structure can vary by version) */
...
.stButton > button[kind="primary"] {{
  background: var(--kmd-accent) !important;
  color: #111827 !important;
}}

.stButton > button[kind="primary"]:hover {{
  filter: brightness(.97);
  box-shadow: 0 16px 24px rgba(249,167,3,.20);
  transform: translateY(-1px);
}}
...
.stButton > button[kind="secondary"] {{
  background: #ffffff !important;
  color: var(--kmd-blue) !important;
  border: 1px solid rgba(6,122,193,.35) !important;
}}

.stButton > button[kind="secondary"]:hover {{
  background: rgba(6,122,193,.06) !important;
  transform: translateY(-1px);
}}
...
[data-testid="stFileUploaderDropzone"] {{
  ...
  border: 1px dashed rgba(6,122,193,.35) !important;
  background: rgba(6,122,193,.03) !important;
}}
...
# ==================== PLOTLY STYLE HELPER (TIDAK MENGUBAH LOGIKA) ====================
def style_plotly(fig):
    fig.update_layout(
        paper_bgcolor="white",
        plot_bgcolor="white",
        font=dict(color=KMD_TEXT),
        title_font=dict(color=KMD_TEXT),
    ...
    fig.update_xaxes(showgrid=True, gridcolor="rgba(229,231,235,1)", zeroline=False)
    fig.update_yaxes(showgrid=True, gridcolor="rgba(229,231,235,1)", zeroline=False)
    return fig

def topbar(title, subtitle):
    st.markdown(f"""
    <div class="kmd-topbar">
...
with st.sidebar:
    ...
    <div class="kmd-brand">
      <div style="display:flex; gap:12px; align-items:center;">
        <div class="kmd-logo">📚</div>
        <div>
          <div class="kmd-brand-title">RDB BBP Bali</div>
          <div class="kmd-brand-sub">Sistem Pendataan Peserta</div>
        </div>
      </div>
      <div style="margin-top:10px; color:{KMD_MUTED}; font-size:12px;">
        Update: <strong>{datetime.now().strftime('%d %B %Y')}</strong>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(
        f"<div style='font-size:12px; color:{KMD_MUTED}; text-align:center;'>"
        f"<strong>Kemendikdasmen Style</strong><br/>"
        f"<span>Elegan • Bersih • Profesional</span>"
        f"</div>",
        unsafe_allow_html=True
    )
...
if menu == "🏠 Dashboard":
    ...
    topbar("RDB BBP Bali", "Ringkasan statistik peserta • Tampilan elegan bernuansa Kemendikdasmen")
    ...
    if data:
        ...
        with col1:
            ...
            fig = px.pie(
                ...
                hole=0.45,
                color_discrete_sequence=[KMD_BLUE, KMD_ACCENT, "#0b5fa8", "#ffbf2f", "#1e293b", "#94a3b8"]
            )
        ...
        with col2:
            ...
            fig.update_traces(marker_color=KMD_BLUE)
            st.plotly_chart(style_plotly(fig), use_container_width=True)
        ...
        with col2:
            ...
            fig.update_traces(marker_color=KMD_ACCENT)
            st.plotly_chart(style_plotly(fig), use_container_width=True)
...
# ==================== FILTER DATA ====================
    ...
    topbar("Filter Data", "Cari & filter data peserta dengan cepat")
...
# ==================== TAMBAH DATA ====================
    ...
    topbar("Tambah Data", "Input data peserta baru ke dalam sistem")
...
# ==================== EDIT DATA ====================
    ...
    topbar("Edit Data", "Muat data berdasarkan ID lalu perbarui")
...
# ==================== HAPUS DATA ====================
    ...
    topbar("Hapus Data", "Penghapusan bersifat permanen—pastikan ID benar")
...
# ==================== REKAPAN ====================
    ...
    topbar("Rekapan", "Statistik lengkap per kategori + unduh laporan")
        ...
        with tab2:
            ...
            if stats:
                ...
                with col2:
                    ...
                    fig.update_traces(line=dict(color=KMD_BLUE), marker=dict(color=KMD_ACCENT))
                    st.plotly_chart(style_plotly(fig), use_container_width=True)
...
# ==================== UPLOAD EXCEL ====================
    ...
    topbar("Upload Excel/CSV", "Unggah data massal dengan validasi otomatis")
